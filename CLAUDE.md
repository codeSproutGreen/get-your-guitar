...
- 테스트로 폰의 설정을 바꿨으면 **바꾼 값만** UI로 되돌린다. `pm clear`는 쓰지 않는다 — 사용자·클라이언트가 폰에서 직접 맞춰 둔 값까지 지운다(2026-09-21에 볼륨 100 · 밝기 71 · 감쇠 100 · 벤딩 2음으로 맞춰져 있는 걸 확인). 슬라이더 값은 `uiautomator dump`로 읽어 가며 맞춘다.
