...
- **M1 "first sound" implemented 2026-09-21 on SDS PC**: core/music, core/synth (Karplus-Strong with loop-filter phase-delay compensation, tuning within 1 cent), core/engine (lock-free SPSC queue), app/audio (AudioTrack low-latency; S10e grants a FAST float track, 16 ms reported latency), app/ui/fretboard. 56 core + 22 app unit tests. What only a human can check (sound, felt latency, multi-touch, upper-register sustain) is open in `docs/verification-checklist.md`. Known characteristic: decay rate grows steeply with pitch (G4 ≈ −56 dB/s); fix candidate for M2 is key-tracking the loop cutoff.
