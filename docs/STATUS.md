...
| 음색 개선 · 누르는 동안만 소리 · 설정 화면 | ✅ 구현 (2026-09-21, SDS PC) | v1.0.0 이후 요청. `specs/2026-09-21-v2-features.md` 6장. `:core` 93 + `:app` 72 테스트, 린트 에러 0. adb로 기능 확인, **소리·연주감은 사람 확인 필요**(체크리스트 "v1.1 후보"). 아직 릴리스하지 않음 |
...
1. **사람이 확인:** 새 빌드(9173525 이후, 폰에 디버그 빌드 설치됨)로 `docs/verification-checklist.md`의 "v1.1 후보" 절. 특히 새 음색이 폰 스피커와 이어폰에서 각각 어떤지, 누르는 동안만 소리의 연주감. PC에서는 `core/build/tone-demo/`의 before/after WAV로 비교(없으면 v2 문서 6장의 명령으로 렌더).
2. 피드백에 따라: 음색이 아직 부족하면 `VoiceCharacter` 값 조정 또는 샘플 음원(B안) 검토. 괜찮으면 **v1.1.0 릴리스**(`versionCode` 2로 올릴 것, README "릴리스 APK" 절차).
