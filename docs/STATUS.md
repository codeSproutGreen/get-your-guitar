...
| 음색 개선 · 뮤트 바 · 설정 화면 | ✅ 구현 (2026-09-21, SDS PC) | v1.0.0 이후 요청. `specs/2026-09-21-v2-features.md` 6·7장. "누르는 동안만 소리"는 설정 스위치로 만들었다가 같은 날 **지판 아래의 뮤트 바**로 바꿨다(누르고 있는 동안만 적용). `:core` 93 + `:app` 85 테스트, 린트 에러 0. adb로 기능 확인, **소리·연주감·뮤트 누른 채 연주는 사람 확인 필요**. 아직 릴리스하지 않음 |
