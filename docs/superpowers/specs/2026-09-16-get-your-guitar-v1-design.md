...
| 항목 | v1에 심어두는 훅 |
|---|---|
| ~~**벤딩**~~ → **v1에 구현됨 (2026-09-21, 사용자 요청으로 앞당김)** | 예고대로 보이스는 그대로 두고 `Command.Bend(string, cents)` + 제스처 계층만 추가했다. 규칙: 짚은 줄의 밴드 안 세로 이동 = 벤딩(위·아래 같음, 데드존 0.1밴드, 0.5밴드에서 최대 200센트), 다른 줄 밴드 진입 = 레이크(벤딩 해제), 레이크로 들어간 줄은 벤딩 안 함, 터치 후 80 ms 뒤에 걸림(빠른 레이크의 첫 줄 보호), 손 떼면 복귀. 벤딩 폭 1음/2음 선택은 M2 설정에 추가 |
