...
| **홀드 서스테인** — 누르고 있는 동안만 소리, 떼면 멈춤 (2026-09-21 추가 요청. 2장의 "자연 감쇠" 결정을 뒤집는다) | 없음. `NoteOff` 커맨드·`Voice.noteOff()`·포인터 소유권 추가 필요. 상세는 `2026-09-21-v2-features.md` 1장 |
| **음색 개선** (2026-09-21 피드백 "소리가 구리다") | 합성 개선은 `KarplusStrongVoice`·`Mixer` 내부, 샘플 교체는 `Voice` 인터페이스. 상세는 `2026-09-21-v2-features.md` 1-2장 |
