...
- 작성일: 2026-09-16
- 상태: 사용자 승인 (2026-09-16). M0에서 compileSdk 36→37 확정 (10장 참고)
...
| 항목 | v1에 심어두는 훅 |
|---|---|
| **벤딩** — 지판 위 세로 드래그를 연속 피치 변화(최대 1~2음)로 해석 | `Voice.setPitch(hz)`가 연속 Hz를 받고 딜레이 라인이 소수점 길이(보간)이므로 **엔진 변경 없이 제스처 계층만 추가** |
...
4. `:app` (compileSdk 37, minSdk 33, Compose, 가로 고정) — "get-your-guitar" 텍스트만 있는 화면
