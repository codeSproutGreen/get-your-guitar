                    ...
                    when {
                        change.changedToDown() -> {
                            if (geometry.isOnBoard(y)) {
                                ...
                                tracker.down(id, geometry.stringAt(y), geometry.fretAt(x, geometry.layout(state.scroll)))
                            } else {
                        ...
                        change.pressed -> {
                            if (id in scrollPointers) {
                                ...
                                tracker.move(id, geometry.stringAt(y), geometry.fretAt(x, geometry.layout(state.scroll)))
                            }
                        }
                    }
                    change.consume()
